 ...
 * 支持「当前会话 / 全部会话」筛选；新建任务归属所选会话（scope 由会话决定）。
 * 模型文本自述与「文件已交付」严格分离——只有 artifacts 表登记（读回校验）才标已校验。
 */
(function () {
  ...
  const sessionSelect = document.getElementById('task-session-select');
  const startBtn = document.getElementById('task-start');
  const filterBar = document.getElementById('task-filter-bar');
  const ctxTag = document.getElementById('task-ctx-tag');
  ...
  let scopeFilter = 'all'; // 'all' | 'current'
  ...
  function scheduleListRefresh() {
    if (listTimer) return;
    listTimer = setTimeout(() => { listTimer = null; refreshList().catch(() => {}); }, 1200);
  }
  function scheduleDetailRefresh(taskId) {
    if (detailTimers.has(taskId)) return;
    detailTimers.set(taskId, setTimeout(() => { detailTimers.delete(taskId); if (expanded.has(taskId)) refreshDetail(taskId).catch(() => {}); }, 350));
  }
  ...
  function fillSessionSelect() {
    const sessions = UI.state.sessions.filter((s) => !s.archived);
    const cur = UI.currentSession();
    ...
    if (cur && sessions.some((s) => s.id === cur.id)) sessionSelect.value = cur.id;
  }

  document.addEventListener('skf:sessions', fillSessionSelect);

  // ── 筛选条 ──
  function renderFilterBar() {
    filterBar.innerHTML = '';
    const scopeAll = document.createElement('button');
    scopeAll.className = 'chip' + (scopeFilter === 'all' ? ' active' : '');
    scopeAll.textContent = '全部会话';
    scopeAll.addEventListener('click', () => { scopeFilter = 'all'; renderFilterBar(); renderList(); });
    const scopeCur = document.createElement('button');
    scopeCur.className = 'chip' + (scopeFilter === 'current' ? ' active' : '');
    scopeCur.textContent = '当前会话';
    scopeCur.addEventListener('click', () => { scopeFilter = 'current'; renderFilterBar(); renderList(); });
    filterBar.append(scopeAll, scopeCur);
    for (const [value, label] of STATE_FILTERS) {
      const b = document.createElement('button');
      b.className = 'chip' + (stateFilter === value ? ' active' : '');
      b.textContent = label;
      b.addEventListener('click', () => { stateFilter = value; renderFilterBar(); renderList(); });
      filterBar.appendChild(b);
    }
    const clear = document.createElement('button');
    clear.className = 'chip';
    clear.textContent = '清除筛选';
    clear.addEventListener('click', () => { scopeFilter = 'all'; stateFilter = 'all'; renderFilterBar(); renderList(); });
    filterBar.appendChild(clear);
  }

  function updateCtxTag() {
    const s = UI.currentSession();
    ctxTag.textContent = scopeFilter === 'current' ? (s ? `会话：${s.name}` : '当前会话') : '全部会话';
  }
  ...
  startBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const session = UI.state.sessions.find((s) => s.id === sid);
      const result = await rpc2('task.start', {
        input: { goal },
        workspaceRoot,
        ...(session ? { sessionId: session.id, scope: session.scope } : {}),
      });
      localStorage.setItem('skf.lastWorkspace', workspaceRoot);
      goalInput.value = '';
      newPanel.open = false;
      expanded.add(result.taskId);
      UI.registerTaskSession(result.taskId, session ? session.id : '');
      await refreshList();
  ...
  async function refreshList() {
    const result = await rpc2('task.list', { limit: 100 });
  ...
  function filteredTasks() {
    ...
    if (scopeFilter === 'current') {
      const cur = UI.currentSession();
      if (cur) list = list.filter((t) => t.sessionId === cur.id);
    }
  ...
  function renderList() {
    ...
    listEl.innerHTML = '';
    if (!tasks.length) {
      listEl.innerHTML = '<div class="empty-block">没有符合条件的任务。</div>';
      return;
    }
  ...
  function stepLineText(task) {
    ...
    switch (task.state) {
      ...
      case 'waiting_provider':
        return counts.modelSteps > 0 ? `模型第 ${counts.modelSteps} 步 · 工具 ${counts.toolCalls} 次` : '准备中（读取任务与记忆上下文）';
  ...
  function renderCard(task) {
    ...
    sub.className = 'sub';
    const sess = UI.state.sessions.find((s) => s.id === task.sessionId);
    sub.textContent = `${task.provider}/${task.model} · ${sess ? sess.name : (task.sessionId || '—')} · ${formatTime(task.createdAt)}`;
    ...
    if (['queued', 'running', 'waiting_provider', 'waiting_approval', 'cancelling'].includes(task.state)) {
      ...
      cancel.addEventListener('click', async () => {
        ...
        try { await rpc2('task.cancel', { taskId: task.id }); }
        catch (error) {
  ...
  function renderDetail(taskId, detail) {
    ...
    if (detail.task.state === 'failed' && detail.task.errorCode) box.appendChild(errorBox(detail.task.errorCode));
    for (const approval of detail.pendingApprovals || []) box.appendChild(approvalRow(taskId, approval));
    if (detail.task.state === 'interrupted') box.appendChild(resumeBox(taskId));
    ...
    if (!artifacts.length) {
      ...
      none.className = 'hint';
      none.textContent = detail.task.state === 'succeeded' ? '这个任务没有登记文件产物。模型自述不等于文件已交付。' : '任务结束后，校验过的文件会列在这里。';
      artBlock.appendChild(none);
    }
    for (const artifact of artifacts) artBlock.appendChild(artifactRow(detail.task.workspaceRoot, artifact));
  ...
  function approvalRow(taskId, approval) {
    ...
    const decide = async (decision) => {
      ...
      ok.disabled = true;
      no.disabled = true;
      try { await rpc2('task.approve', { approvalId: approval.id, inputHash: approval.inputHash, decision }); }
      catch (error) {
        const fe = friendlyError(error);
        alert(fe.text + '\n' + fe.next);
      }
  ...
  function resumeBox(taskId) {
    ...
    go.addEventListener('click', async () => {
      ...
      try {
        ...
        hint.textContent = '恢复被阻止：' + friendlyError(block.code || 'TASK_INTERRUPTED').next;
      } catch (error) {
  ...
  async function renderTaskBudget(box, taskId) {
    ...
    try {
      ...
      if (!task || task.calls === 0) { line.textContent = '本任务还没有模型调用。'; return; }
      const parts = [
    ...
    } catch { line.textContent = '费用状态暂时读不到。'; }
  }
  ...
  async function renderTimeline(box, taskId) {
    ...
    try {
      ...
      list.innerHTML = '';
      if (!events.length) { list.textContent = '还没有事件。'; return; }
    ...
    } catch { list.textContent = '事件暂时读不到。'; }
  }
