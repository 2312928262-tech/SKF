...
/**
 * 记忆视图：按当前会话 scope 查、看来源/确认状态、更正、归档/恢复、备份。
 * scope 持续可见（绑定当前会话）；跨范围浏览需显式切换会话。主流程不出现数据库路径。
 */
(function () {
  ...
  const scopeTag = document.getElementById('mem-scope-tag');
  const scopeHint = document.getElementById('mem-scope-hint');
  ...
  const TRUST_TEXT = { user_confirmed: '用户确认', tool_observed: '工具观察', legacy: '历史记录', candidate: '候选' };
  const KIND_TEXT = { fact: '事实', preference: '偏好', lesson: '经验', episode: '经历', reference: '资料', decision: '决定' };
  ...
  function renderScopeTag() {
    ...
    scopeTag.textContent = scope ? 'scope: ' + scope : 'scope: —';
    scopeTag.title = scope || '';
    scopeHint.textContent = scope
      ? `当前会话范围（${scope}）持续显示；记忆检索与写回绑定此范围，与其他会话隔离。`
      : '未选择会话；记忆检索使用全局范围。';
  }
  ...
  async function renderHeader() {
    ...
    try {
      ...
      stat.textContent = fe.code === 'MEMORY_UNAVAILABLE' ? '记忆主档暂不可用：不阻塞对话与任务；写回会排队自动补投。' : `记忆状态读取失败：${fe.text}`;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const scope = currentScope();
      const data = await rpc2('memory.search', {
        query, limit: 12, includeArchived: includeArchived.checked,
        ...(scope ? { scope } : {}),
      });
      lastHits = data.hits || [];
      renderHits(data);
  ...
  function renderHit(hit, index) {
    ...
    text.className = 'text';
    text.textContent = hit.text.length > 280 ? hit.text.slice(0, 280) + '…' : hit.text;
    const badges = document.createElement('div');
    badges.className = 'badges';
    const badgeList = [TRUST_TEXT[hit.trust] || hit.trust, KIND_TEXT[hit.kind] || hit.kind];
    ...
    toggle.addEventListener('click', () => {
      ...
      if (detail) { detail.remove(); toggle.textContent = '详情'; }
      else { card.appendChild(renderDetail(hit, index)); toggle.textContent = '收起'; }
    });
  ...
  function renderDetail(hit, index) {
    ...
    if (hit.status !== 'archived') {
      ...
      archive.title = '归档后默认搜索不再出现，可随时恢复；不删除条目。';
      ...
      restore.addEventListener('click', async () => {
        ...
        try { await rpc2('memory.restore', { id: hit.id }); rerunSearch(); }
        catch (error) {
          const fe = friendlyError(error);
          alert(fe.text + '\n' + fe.next);
  ...
  document.addEventListener('skf:session-switched', () => {
    renderScopeTag();
    renderHeader().catch(() => {});
    if (lastHits.length) { lastHits = []; results.innerHTML = '<div class="empty-block">会话已切换，输入关键词在当前范围重新搜索。</div>'; }
  });
